...
public partial class Mein
{
    ...
    private async Task LoadAsync()
    {
        ...
        helperHasProfileImage = appUser.ProfileImageUpdatedUtc.HasValue || appUser.ProfileImageData is { Length: > 0 };
    ...
    protected string? GetCurrentHelperImageUrl()
        => currentAppUserId.HasValue && helperHasProfileImage
            ? BuildProfileImageUrl(currentAppUserId.Value, profileImageUpdatedUtc)
            : null;

    private static string BuildProfileImageUrl(Guid userId, DateTime? updatedUtc)
        => updatedUtc.HasValue
